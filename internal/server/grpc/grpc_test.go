...
func TestCutter(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			g := New(a, c)
			got, err := g.h.Cutter(context.Background(), tt.request)
...
func TestRedirect(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			pMocks: func(m *mock) {
				gomock.InOrder(
					m.cutter.EXPECT().GetKeyByValue(gomock.Any(), gomock.Any()).Return("http://ya.ru/", nil).MaxTimes(0),
				)
			},
			w: want{
				response: nil,
...

		{
			name: "wrong url",
			request: &pb.RedirectRequest{
				Shorten: "http://localhost:8080trtrtr",
			},
			pMocks: func(m *mock) {
				gomock.InOrder(
					m.cutter.EXPECT().GetKeyByValue(gomock.Any(), gomock.Any()).Return("http://ya.ru/", nil).MaxTimes(0),
...

		{
			name: "positive",
			request: &pb.RedirectRequest{
				Shorten: "http://localhost:8080/trtrtr",
			},
			pMocks: func(m *mock) {
				gomock.InOrder(
					m.cutter.EXPECT().GetKeyByValue(gomock.Any(), gomock.Any()).Return("http://ya.ru/", nil).MaxTimes(1),
				)
			},
			w: want{
				response: &pb.RedirectResponse{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			g := New(a, c)
			got, err := g.h.Redirect(context.Background(), tt.request)
